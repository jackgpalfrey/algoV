...
const ComputerVisualiser: React.FC = () => {
	...
	function visualiseMemory() {
		let stuff = memArray.map((value, index) => {
			...
			let addressingModeName = getAddressingModeFromOpcode(
				value,
				instructionName
			);
			...
			if (memFormat === 'BIN') {
				...
				return (
					<div
						className={`memoryLocation memFormat-${memFormat} ${
							isCurrentPC ? 'activePC' : null
						}`}
						title={`Address: ${index + 256 * memPage} (${toHex(
							index + 256 * memPage,
							true
						)})`}>
						<p>{binRep1}</p>
	...
	function setPage(value: number) {
		if (typeof value !== 'number' || value === NaN || value < 0 || value > 255)
			return;
		setMemPage(value);
		openPage(value);
	}
		...
		<div className='compute_container'>
			<div className='compute-inner-container'>
				...
				<div className='registryBox'>
					...
					<br />
					<p>
						<span className='registerName'>Program Counter:</span> {PC}
					</p>
					<p>
						<span className='registerName'>Stack Pointer:</span> {SP}
					</p>
					<br />
					<p>
						<span className='registerName'>Accumulator:</span> {A}
					</p>
					<p>
						<span className='registerName'>X Register:</span> {X}
					</p>
					<p>
						<span className='registerName'>Y Register:</span> {Y}
					</p>
					<h3 className='registerTitle'>Flags</h3>
					<br />
					<p>
						<span className='registerName'>Carry Flag:</span>{' '}
						{flags.C ? '1' : '0'}
					</p>
					<p>
						<span className='registerName'>Zero Flag:</span>{' '}
						{flags.Z ? '1' : '0'}
					</p>
					<p>
						<span className='registerName'>Interupt Disable Flag:</span>{' '}
						{flags.I ? '1' : '0'}
					</p>
					<p>
						<span className='registerName'>Decimal Mode Flag:</span>{' '}
						{flags.D ? '1' : '0'}
					</p>
					<p>
						<span className='registerName'>Break Flag:</span>{' '}
						{flags.B ? '1' : '0'}
					</p>
					<p>
						<span className='registerName'>Overflow Flag:</span>{' '}
						{flags.V ? '1' : '0'}
					</p>
					<p>
						<span className='registerName'>Negative Flag:</span>{' '}
						{flags.N ? '1' : '0'}
					</p>
				</div>

				<div className='screen'>{drawScreen()}</div>
				...
				<div className='memoryBox'>
					...
					<hr />
					<div className='memoryContainer'>{visualiseMemory()}</div>
				</div>
			</div>
		</div>
	);
