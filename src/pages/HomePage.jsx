...
function HomePage() {
	...
	let changeItems = changeLogItems.map((value, idx) => {
		...
		if ((!dontDisplay && !displayAfter) || timeUntil <= 0)
			return (
				<Item
					key={idx}
					isOpen={isAllOpen}
					changes={value.modifications}
					date={
						value.isNext ? `${text.plannedForDate} ${value.date}` : value.date
					}
					version={value.isNext ? text.nextUpdateVersionKey : value.version}
					title={value.title}
				>
					{' '}
					<p>{value.info}</p>{' '}
	...
	return (
		<div className='changeLog'>
			<div className='changeLogMenuItems'>
				<div
					className='openAllChangesButton clickable'
					onClick={() => setIsAllOpen(!isAllOpen)}
				>
					{isAllOpen ? text.closeAllButton : text.openAllButton}
				</div>
				<img
				...
				>
					{isNewestFirst
						? text.orderOldestToNewestButton
						: text.orderNewestToOldestButton}
				</div>
			</div>
			<Container
				titleBgColor=''
